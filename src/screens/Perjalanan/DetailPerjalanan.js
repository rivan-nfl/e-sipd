...
import { StyleSheet, Text, View, ScrollView, Modal, Linking, Alert, Dimensions, Image } from 'react-native'
...
const DetailPerjalanan = ({ route, navigation }) => {
    ...
    const handleChooseImage = () => {
        ...
        launchImageLibrary(options, async (res) => {
            if (res.didCancel) {
                ...
                const perjalanan = await handleSelesaiPerjalanan()

                if (perjalanan) {
                    const response = await axios({
                        method: 'POST',
                        url: `${baseUrl}/upload-asset-perjalanan`,
                        data: {
                            perjalananId: perjalanan,
                            file: {
    ...
    const handleSelesaiPerjalanan = async () => {
        try {
            const response = await setSelesaiPerjalanan(token, params.id)
            const res = await getAllPerjalanan(token)
            dispatch({ type: 'SAVE_PERJALANAN', data: res.data.data })
    ...
    const getAsset = () => {
        axios({
            method: 'GET',
            url: `${baseUrl}/e-sipd/asset/${params.id}`,
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
            .then(res => {
                setImage(res.data.data.asset)
            })
            .catch(err => {
    ...
    useEffect(() => {
        ...
        if (params.status === 'finished') getAsset()
    }, [])
            ...
            <Layout title='Detail Perjalanan'>
                <View style={{ paddingVertical: 20, width: '95%', flex: 1 }}>
                    <ScrollView style={{ backgroundColor: COLORS.WHITE }} showsVerticalScrollIndicator={false}>
                        ...
                        {/* Image */}
                        {image && <>
                            <Text style={styles.menuTxt}>Bukti</Text>
                ...
                {((status == 'approved' || status == 'finished') && userRole == 'anggota') && (
                    <View style={styles.footer}>
                        ...
                        {status != 'finished' && (
                            <CustomButton title='Selesai Perjalanan' buttonStyle={{ width: '100%' }} onPress={() => {
                                Alert.alert(
                                    "Sukses",
                                    "Berhasil Membuat Anggota",
                                    [
                                        { text: "Pilih Foto Galeri", onPress: handleChooseImage },
                                        { text: "Ambil Foto", onPress: handlePickImage },
                                        { text: "Kembali", onPress: null }
                                    ]
                                );
                            }} />
                        )}
            ...
            >
                <View style={styles.modalView}>
                    <View style={styles.modalCard}>
                        ...
                        />
                        <View style={{ flexDirection: 'row', alignSelf: 'center', justifyContent: 'space-around', alignItems: 'center', marginTop: 15, width: '100%' }}>
                            <CustomButton title='Cancel' buttonStyle={[styles.deleteBtn, { width: '45%' }]} style={styles.editBtnTxt} onPress={() => setModalVisible(!modalVisible)} />
                            <CustomButton title='Confirm' buttonStyle={[styles.editBtn, { width: '45%', marginBottom: 0 }]} onPress={() => handleUpdatePerjalanan('rejected')} />
                        </View>
                    </View>
                </View>
            </Modal>
